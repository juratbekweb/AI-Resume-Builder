...
const templates = [
  { id: 1, name: 'Modern Executive', category: 'Executive', accent: 'from-cyan-500 to-blue-600', bg: 'from-cyan-500/10 to-blue-500/5', border: 'border-cyan-400/20', preview: 'Clean hierarchy, serif headings, subtle dividers. Perfect for C-suite and leadership roles where gravitas matters.', features: ['2-column layout', 'Executive summary box', 'Board-ready format'] },
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-12 sm:px-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold text-white">{t.templatesTitle}</h1>
            </div>
          </div>
          <p className="text-slate-400 max-w-2xl">{t.templatesDesc}</p>
        </div>
      </div>
          ...
          {categories.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)}
              className={`rounded-full border px-4 py-1.5 text-sm font-medium transition-all ${
                filter === cat ? 'border-pink-400/60 bg-pink-400/15 text-pink-300' : 'border-white/10 bg-white/5 text-slate-400 hover:border-white/20 hover:text-white'
              }`}>
              {cat}
            </button>
          ))}
              ...
              >
                ...
                <div className="mt-3 flex gap-2">
                  <button onClick={e => { e.stopPropagation(); setSelected(tpl.id); }}
                    className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-white/10 py-2 text-xs font-medium text-slate-300 hover:border-white/20 hover:text-white transition-colors">
                  ...
                  <button onClick={e => { 
                    ...
                    className={`flex flex-1 items-center justify-center gap-1.5 rounded-lg py-2 text-xs font-medium transition-all bg-gradient-to-r ${tpl.accent} text-white hover:opacity-90`}>
        ...
        {modal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              className={`relative w-full max-w-lg rounded-2xl border ${modal.border} bg-slate-900 p-8`}
              onClick={e => e.stopPropagation()}>
              <button onClick={() => setSelected(null)} className="absolute right-4 top-4 rounded-lg border border-white/10 p-1.5 text-slate-400 hover:text-white"><X className="h-4 w-4" /></button>
              <div className={`mb-6 h-40 rounded-xl bg-gradient-to-br ${modal.accent} opacity-25`} />
              <h2 className="text-xl font-bold text-white mb-1">{modal.name}</h2>
              <span className="rounded-full border border-white/10 px-2.5 py-0.5 text-xs text-slate-400">{modal.category}</span>
              <p className="mt-4 text-sm text-slate-300 leading-7">{modal.preview}</p>
              <div className="mt-4 flex flex-wrap gap-2">
                {modal.features.map(f => <span key={f} className="flex items-center gap-1.5 rounded-full bg-white/5 border border-white/10 px-3 py-1 text-xs text-slate-300"><Check className="h-3 w-3 text-emerald-400" />{f}</span>)}
              </div>
              <button onClick={() => { 
                setUsed(modal.id); 
                setTimeout(() => router.push('/features/preview'), 600);
              }}
                className={`mt-6 w-full rounded-xl bg-gradient-to-r ${modal.accent} py-3 text-sm font-semibold text-white hover:opacity-90 transition-opacity`}>

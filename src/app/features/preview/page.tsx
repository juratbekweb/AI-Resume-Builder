...
function Field({ label, value, onChange, multiline = false, rows = 2 }: { label: string; value: string; onChange: (val: string) => void; multiline?: boolean; rows?: number }) {
  return (
    <div>
      <label className="mb-1 block text-[11px] font-semibold uppercase tracking-widest text-foreground-secondary">{label}</label>
      {multiline ? (
        <textarea value={value} onChange={e => onChange(e.target.value)} rows={rows}
          className="w-full resize-none rounded-lg border border-border bg-surface-elevated/70 px-3 py-2 text-sm text-foreground focus:border-orange-400/50 focus:outline-none" />
      ) : (
        <input value={value} onChange={e => onChange(e.target.value)}
          className="w-full rounded-lg border border-border bg-surface-elevated/70 px-3 py-2 text-sm text-foreground focus:border-orange-400/50 focus:outline-none" />
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-transparent pb-10">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-10 sm:px-8">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold text-foreground">{t.previewTitle}</h1>
            </div>
          </div>
          <p className="text-foreground-secondary max-w-2xl">{t.previewDesc}</p>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-8">
        <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
          {/* Form panel */}
          <div className="rounded-2xl border border-border bg-surface-elevated overflow-hidden backdrop-blur-sm">
            {/* Tabs */}
            <div className="flex border-b border-border">
              {(['personal','experience','skills'] as const).map(tKey => (
                <button key={tKey} onClick={() => setTab(tKey)}
                  className={`flex-1 py-3 text-xs font-semibold uppercase tracking-wider transition-colors ${
                    tab === tKey ? 'border-b-2 border-orange-400 text-orange-300' : 'text-foreground-secondary hover:text-slate-300'
                  }`}>
          ...
          <motion.div
            ...
            initial={{ opacity: 0.8 }}
            animate={{ opacity: 1 }}
            className="rounded-2xl border border-border bg-white p-8 text-slate-900 shadow-2xl min-h-[600px]"
          >
            ...
            {r.summary && (
              <div className="mb-5">
                ...
                <p className="text-sm text-foreground-secondary leading-6">{r.summary}</p>
              </div>
            )}
            <div className="mb-5">
              ...
              {[{role:r.exp1Role,company:r.exp1Company,period:r.exp1Period,desc:r.exp1Desc},{role:r.exp2Role,company:r.exp2Company,period:r.exp2Period,desc:r.exp2Desc}].map((e,i) => e.role && (
                <div key={i} className="mb-4">
                  <div className="flex items-baseline justify-between">
                    ...
                    <span className="text-xs text-foreground-secondary">{e.period}</span>
                  </div>
                  <div className="text-xs font-medium text-orange-600 mb-1">{e.company}</div>
                  <pre className="whitespace-pre-wrap text-xs text-foreground-secondary leading-5">{e.desc}</pre>
                </div>
              ))}
            </div>
            {r.skills && <div className="mb-4"><h3 className="mb-1 text-[10px] font-black uppercase tracking-[0.2em] text-slate-300">{t.skillsTab}</h3><p className="text-sm text-foreground-secondary">{r.skills}</p></div>}
            {r.education && <div><h3 className="mb-1 text-[10px] font-black uppercase tracking-[0.2em] text-slate-300">{t.education}</h3><p className="text-sm text-foreground-secondary">{r.education}</p></div>}
          </motion.div>
        </div>
      </div>
    </div>
